...
export function assessmentFromRecord(trace?: AgentTrace, flow?: TvFlowMemoryDocument, expectedAgentId?: string): Assessment {
  ...
  if (trace) {
    ...
    for (const step of trace.llmSteps) add({ kind: "context", timestamp: step.timestamp, text: JSON.stringify({ systemMessages: step.systemMessages, messages: step.messages, text: step.text, toolCalls: step.toolCalls }), source: `telemetry:${trace.sessionId}:llm:${step.stepNumber}` });
    for (const [index, event] of trace.events.entries()) add({ kind: "context", timestamp: event.timestamp,
      text: JSON.stringify({ type: event.type, message: event.message, data: event.data }), source: `telemetry:${trace.sessionId}:event:${index}` });
