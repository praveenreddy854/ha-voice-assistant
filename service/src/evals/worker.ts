...
export async function runWorker(job: EvalJob, store = new EvalStore()): Promise<void> {
  ...
  try {
    ...
    job.status = "running"; await store.write("jobs", job);
    ...
    if (job.mode === "calibrate") {
      ...
      if (job.mode === "recorded") {
        ...
        batch = await runner.recorded("tv", job.sessionIds.map(id => () => loadRecordedAssessment(id)), controller.signal);
      } else {
        ...
        batch = await runner.simulated(adapter, controller.signal, { scheduledDay: job.scheduledDay, scenarioIds: job.scenarioIds });
      }
    ...
    await store.write("jobs", job); await release();
