...
const jobSchema = z.discriminatedUnion("mode", [
  ...
  z.object({ mode: z.literal("recorded"), sessionIds: z.array(z.string().regex(/^[a-zA-Z0-9_-]+$/)).min(1).max(100) }).strict(),
  z.object({ mode: z.literal("calibrate") }).strict(),
]);
export function createEvalRouter(supervisor: EvalSupervisor) {
  ...
  router.get("/dashboards/evals/browser.js", (_req, res) => {
    res.sendFile(path.join(__dirname, __filename.endsWith(".ts") ? "../../dist/evals/browser.js" : "browser.js"));
  });
  router.get("/api/evals", async (req, res, next) => {
    try {
      const [allRuns, batches, alerts, groups, calibrations, jobs] = await Promise.all([
      ...
      const day = localDay();
      res.json({ timezone: EVAL_TIMEZONE, baseline: { from: shiftDay(day, -7), to: shiftDay(day, -1), minimumSamples: 3 },
  ...
  router.post("/api/evals/jobs", async (req, res) => {
    ...
    if (origin && new URL(origin).host !== req.get("host")) { res.status(403).json({ error: "Cross-origin eval submission rejected" }); return; }
