...
```sh
cd service && npx tsc
cd ../ha-voice-assistant && npm run test:e2e -- e2e/eval-history.spec.ts e2e/eval-metrics.spec.ts e2e/eval-scoring.spec.ts e2e/eval-schedules.spec.ts
```
